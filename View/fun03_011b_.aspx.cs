...
public partial class View_fun03_011b : System.Web.UI.Page
{
    ...
    protected void Query()
    {
        ...
        sql_ori = "Select plantid,pointx,pointy, Plant_name,segment,highway_name,start,[end],direction,cast(start as varchar)+'~'+cast([end] as varchar) as [range],Plant_number,florescence,FlowerColor,case when Img <>'' then ('..\\Attachments\\Plants\\'+Img) else '' end as Img,case when Img <>'' then ('..\\Attachments\\Plants\\'+Img) else '' end as lo,Case When Img='' Then '' Else '' End As image_memo From Plants Where 1=1 ";
        sql = "Select plantid,pointx,pointy, Plant_name,segment,highway_name,start,[end],direction,cast(start as varchar)+'~'+cast([end] as varchar) as [range],Plant_number,florescence,FlowerColor,case when Img <>'' then ('..\\Attachments\\Plants\\'+Img) else '' end as Img,case when Img <>'' then ('..\\Attachments\\Plants\\'+Img) else '' end as lo,Case When Img='' Then '' Else '' End As image_memo From Plants Where 1=1 ";
	    ...
	    sql_ori += " and (Segment = '" + vc2 + "' or ((Segment is NULL or Segment = '')";
            sql_ori += " and PointX <= (select max_x from Engineering_road_scope where start_end = '" + value_class2 + "') ";
            sql_ori += " and PointX >= (select min_x from Engineering_road_scope where start_end = '" + value_class2 + "') ";
            sql_ori += " and PointY <= (select max_y from Engineering_road_scope where start_end = '" + value_class2 + "') ";
            sql_ori += " and PointY >= (select min_y from Engineering_road_scope where start_end = '" + value_class2 + "'))) ";
	    sql += " and (Segment = '" + vc2 + "' or ((Segment is NULL or Segment = '')";
            sql += " and PointX <= (select max_x from Engineering_road_scope where start_end = '" + value_class2 + "') ";
            sql += " and PointX >= (select min_x from Engineering_road_scope where start_end = '" + value_class2 + "') ";
            sql += " and PointY <= (select max_y from Engineering_road_scope where start_end = '" + value_class2 + "') ";
            sql += " and PointY >= (select min_y from Engineering_road_scope where start_end = '" + value_class2 + "'))) ";
	    sql_where += " and (Segment = '" + vc2 + "' or ((Segment is NULL or Segment = '')";
            sql_where += " and PointX <= (select max_x from Engineering_road_scope where start_end = '" + value_class2 + "') ";
            sql_where += " and PointX >= (select min_x from Engineering_road_scope where start_end = '" + value_class2 + "') ";
            sql_where += " and PointY <= (select max_y from Engineering_road_scope where start_end = '" + value_class2 + "') ";
            sql_where += " and PointY >= (select min_y from Engineering_road_scope where start_end = '" + value_class2 + "'))) ";
        } else if (!value_class1.Equals("ALL"))
	{
	    sql_ori += " and (Office = '" + value_class1 + "' or ((Office is NULL or Office = '')";
            sql_ori += " and PointX <= (select max_x from Engineering_road_scope where start_end = '" + value_class1 + "') ";
            sql_ori += " and PointX >= (select min_x from Engineering_road_scope where start_end = '" + value_class1 + "') ";
            sql_ori += " and PointY <= (select max_y from Engineering_road_scope where start_end = '" + value_class1 + "') ";
            sql_ori += " and PointY >= (select min_y from Engineering_road_scope where start_end = '" + value_class1 + "'))) ";
	    sql += " and (Office = '" + value_class1 + "' or ((Office is NULL or Office = '')";
            sql += " and PointX <= (select max_x from Engineering_road_scope where start_end = '" + value_class1 + "') ";
            sql += " and PointX >= (select min_x from Engineering_road_scope where start_end = '" + value_class1 + "') ";
            sql += " and PointY <= (select max_y from Engineering_road_scope where start_end = '" + value_class1 + "') ";
            sql += " and PointY >= (select min_y from Engineering_road_scope where start_end = '" + value_class1 + "'))) ";
	    sql_where += " and (Office = '" + value_class1 + "' or ((Office is NULL or Office = '')";
            sql_where += " and PointX <= (select max_x from Engineering_road_scope where start_end = '" + value_class1 + "') ";
            sql_where += " and PointX >= (select min_x from Engineering_road_scope where start_end = '" + value_class1 + "') ";
            sql_where += " and PointY <= (select max_y from Engineering_road_scope where start_end = '" + value_class1 + "') ";
            sql_where += " and PointY >= (select min_y from Engineering_road_scope where start_end = '" + value_class1 + "'))) ";
	}
	...
	if (Session["xy02_011a"] != null) {
        ...
        map_url = map_url.Replace("fun03_011b_.aspx", "gmap02_011a_.aspx");
	}
