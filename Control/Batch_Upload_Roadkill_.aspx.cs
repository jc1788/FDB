...
public partial class Control_Batch_Upload_Roadkill : System.Web.UI.Page
{
		  ...
		  SheetName = Sheets.Rows[0][2].ToString().Replace("'","");
			...
			cmd.Parameters.AddWithValue("value_uid",value_uid);
                    ...
                    while (dr.Read())
                    {
                        ...
                        string sql_insert = "";
                            ...
                            if (isNewFormat)
                            {
                                sql_insert = "Insert Into Roadkill_tmp(id,site_ch,highway_id,direction,milestone,x,y,TM2_X,TM2_Y,date,range,type,weather,animal,detail_animal,collecter_ch,species,deduce_species,file1,file2,file3,file4,file5,path1,transfer,note,uid,Sensitive_level,upload_file,upload_date,image_file) values(";
                            }
                            else
                            {
                                sql_insert = "Insert Into Roadkill_tmp(id,site_ch,highway_id,direction,milestone,x,y,TM2_X,TM2_Y,date,range,type,weather,animal,detail_animal,collecter_ch,species,deduce_species,file1,path1,transfer,note,uid,Sensitive_level,upload_file,upload_date,image_file) values(";
                            }
				...
				|| (value_species.Contains("鳥") && !value_animal.Contains("鳥"))))
					...
					cmd.Parameters.AddWithValue("value_uid",value_uid);
                                ...
                                if (!value_highway_id.Equals("") && !value_direction.Equals("") && !value_mileage.Equals(""))
                                {
            ...
            conn.Close();
                ...
                //Response.Write("<script>alert('上傳資料完成!');location.href='RoadKill_tmp.aspx?counts_error=" +counts_error+"'; </script>");
                    ...
                    redirect_str = "RoadKill_tmp_.aspx?counts_error=" + counts_error + "&counts_ok=" + counts_ok;
    ...
    protected void Upload_Month_NoRecord(object sender, EventArgs e)
    {
        ...
        Response.Write("<script>alert('設定本月無資料完成!');location.href='Batch_Upload_Roadkill_.aspx'; </script>");
