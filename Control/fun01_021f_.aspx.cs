...
public partial class View_fun01_021f : System.Web.UI.Page
{
    ...
    protected void Page_Load(object sender, EventArgs e)
    {
         ...
         map_url = map_url.Replace("control/fun01_021f_.aspx", "view/gmap01_021f.aspx");
    ...
    {
        string sql = " SELECT distinct  siteid, CONVERT (char(10), date1, 126) AS invest_date, highway_id, site_ch, Round(x,2) as x , Round(y,2) as y, TM2_X, TM2_Y, Short_Name , Chinese_Name , accepted_name_code , '../Attachments/Occurrence/'+file1 AS lo FROM table_1 Where 1=1 ";
        ...
        if (!String.IsNullOrEmpty(value_KeyWords))
        {
            if (!value_KeyWords.Equals(""))
            {
                ...
                sql += "    or ScientificName Like '%" + value_KeyWords + "%' )";

                sql_xy += " And ( Chinese_Name Like '%" + value_KeyWords + "%' ";
                sql_xy += "    or ScientificName Like '%" + value_KeyWords + "%' )";
            }
        }
