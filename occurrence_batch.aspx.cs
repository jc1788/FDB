...
public partial class occurrence_batch : System.Web.UI.Page
{
		...
		return;
            ...
            switch (SheetName)
            {
                ...
                case "occurrence":
                    ...
                    data_string = "生態調查紀錄: ";
		    string sql_delete = "Begin Delete Table_2 Where uid = @value_uid; Delete Table_3 Where uid = @value_uid; End;";
                    try
                    {
                        conn_insert.Open();
			...
			cmd.Parameters.AddWithValue("value_uid",value_uid);
                    ...
                    while (dr.Read())
                    {
                        ...
                        string sql_insert = "Insert Into table_2(Chinese_name,short_name,[group],Density,way_ch,date1,x,y,site_city,site_area,site_ch,highway_id,direction,mileage,habit,file1,Collector_ch,plan_name,invest_company,notes,Sensitive_level,accepted_name_code,sec_record,inaccuracy,siteid,TM2_X,TM2_Y,full_name,uid) Values(";
			...
			// 2024/1/24 修正日期輸入格式問題
			    ...
			    cmd = new SqlCommand();
			    if (String.IsNullOrEmpty(value_Chinese_name))
			    {
				error_note += "物種名";
				error = 1;
			    }
				cmd.Parameters.AddWithValue("value_Chinese_name",value_Chinese_name);
			    if (String.IsNullOrEmpty(value_Short_name))
			    {
				if (error == 1)
				{
				    error_note += "、";
			    ...
			    if (String.IsNullOrEmpty(value_Density))
			    {
				if (error == 1)
				{
				    error_note += "、";
			    ...
			    if (String.IsNullOrEmpty(value_date1))
			    {
				if (error == 1)
				{
				    error_note += "、";
			    ...
			    if (String.IsNullOrEmpty(value_x))
			    {
				if (error == 1)
				{
				    error_note += "、";
				...
				cmd.Parameters.AddWithValue("value_x",value_x);
			    if (String.IsNullOrEmpty(value_y))
			    {
				if (error == 1)
				{
				    error_note += "、";
				...
				cmd.Parameters.AddWithValue("value_site_ch",value_site_ch);
			    if (String.IsNullOrEmpty(value_Collector_ch))
			    {
				if (error == 1)
				{
				    error_note += "、";
